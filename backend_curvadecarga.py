...
def _read_any(uploaded_or_path):
    ...
    if isinstance(uploaded_or_path, str):
        ...
        if path.endswith(".csv"):
            df = pd.read_csv(uploaded_or_path, dtype=str, header=None)
        else:
...
def _guess_cols(df: pd.DataFrame):
    ...
    c_dt = find([r"^fecha.?y.?hora$", r"fecha.?hora", r"datetime", r"timestamp", r"instante"])
...
def normalize_curve_simple(uploaded, origin="archivo") -> tuple[pd.DataFrame, pd.DataFrame, str]:
    ...
    try:
        ...
        if c_dt:
            ...
            # --- Corrección específica para "24:00" o "24:00:00" ---
            #hora_raw = hora_raw.replace({"24:00": "23:59", "24:00:00": "23:59"})
            #hora_raw = hora_raw.replace({"24:00:00": "24:00"})
            print(hora_raw)
